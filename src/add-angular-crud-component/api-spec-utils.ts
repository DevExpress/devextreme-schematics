...
export function getProperty(model: ApiSpecModel, name: string): PropertyModel {
  const index = model.properties.findIndex((item) => item.name === name);
...
function getActionUrlByMethod(model: ApiSpecModel, method: string): string {
  for (const name in model.actions) {
...
export function getKeyDataProperty(model: ApiSpecModel): any {
  const properties: PropertyModel[] = model.properties.filter((item) => item.isKey === true);
  if (properties.length > 1) {
    return properties.map((item) => item.name);
  }
  if (properties.length === 1) {
...
export function getDataLookups(model: ApiSpecModel): LookupModel[] {
  const properties: PropertyModel[] = model.properties.filter((item, pos) => {
